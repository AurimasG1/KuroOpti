...
namespace KuroOpti.Services.Implementations
{
	...
	{
		private const string GoogleGeocodingUrl = "https://maps.googleapis.com/maps/api/geocode/json";
		...
		public async Task<(decimal Latitude, decimal Longitude)> GeocodeAsync(string address, string municipality)
		{
			...
			string url = $"{GoogleGeocodingUrl}?address={Uri.EscapeDataString(query)}&key={_apiKey}&region=lt&language=lt";
			...
			try
			{
				...
				if (status != "OK")
				{
					_logger.LogWarning("Google geocoding failed for: {Query} — status: {Status}", query, status);
					return (0, 0);
				}

				JsonElement location = root
		...
		{
			return s.Contains(" g.") || s.Contains(" pr.") || s.Contains(" al.") ||
			       s.Contains(" pl.") || s.Contains(" aplinkl.") || s.Contains(" k.");
		}
	}
}
